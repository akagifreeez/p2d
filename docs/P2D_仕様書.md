...
| F-012  | QRコード接続     | QRコードスキャンによる接続    | 中     | ✅ 実装済み (2026-09-13: 表示=p2d://join QR、読み取り=jsQR) |
| F-013  | 接続履歴         | 過去の接続先を記憶            | 低     | ✅ 実装済み (直近8件、localStorage) |
| F-014  | 複数ビューア対応 | 1つのホストに複数視聴者が接続 | 中     | ✅ (Full Mesh) |
...
| 機能ID | 機能名         | 説明                                    | 優先度 | 状態 |
| ------ | -------------- | --------------------------------------- | ------ | ---- |
| F-050  | プレゼンス表示 | 画面共有中にDiscordステータスを更新     | 高     | ✅ 実装済み (2026-09-13) |
| F-051  | 参加ボタン     | Discordから直接画面共有セッションに参加 | 高     | ✅ 実装済み (p2d://join/<コード> ディープリンク) |
| F-052  | 招待機能       | DiscordフレンドをP2Dセッションに招待    | 中     | 未実装 |

> [!NOTE]
> F-050/F-051の表示にはDiscord Developer Portalで作成した**Application IDが必要** (設定モーダルまたは `--p2d-discord-client-id=` 起動引数で設定)。未設定時は機能が無効になりアプリへの影響はない。参加ボタンは `p2d://` プロトコルハンドラ (初回起動時にHKCU登録) 経由でアプリを起動し自動参加する。
...
> 進捗は実装状況に合わせて更新 (最終同期: 2026-09-12)

### Phase 1: MVP（4週間） — ✅ 完了

#### Week 1-2: 基盤構築
- [x] プロジェクト初期化（Tauri + React）
- [x] シグナリングサーバー実装
- [x] 基本的なWebRTC接続確立
...
#### Week 7
- [x] DataChannel実装
- [x] リモートマウス操作（オプション） — キーボード入力・操作権限管理 (F-021/F-022)・クリップボード同期 (F-023) も実装済み
...
#### Week 9
- [ ] クロスプラットフォームテスト
- [x] パッケージング — MSI / NSISインストーラのビルドを確認 (`npm run tauri build`)
- [x] ドキュメント整備
...
| バージョン | 日付       | 変更内容                                | 著者 |
| ---------- | ---------- | --------------------------------------- | ---- |
| 1.3        | 2026-09-13 | Discord Rich Presence (F-050/F-051) 実装、実機2台E2E達成を反映 | AI   |
| 1.2        | 2026-09-12 | ロードマップ進捗を実装状況に同期        | AI   |
