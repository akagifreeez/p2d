...
*   **WSリレーモード (2026-09-13実装・実機検証済み)**: WebRTC非対応エンジニアリング(Linux WebKitGTK)向けフォールバック。2モード: **H264/fMP4+MSE (品質モード)** — ホストがWebCodecs VideoEncoder→mp4-muxer(fragmented)→box単位でWS送信、ゲストはMediaSourceで`<video>`再生(~2.5Mbps、JPEG比1/5以下の帯域・MSE分の1-2秒遅延) / **JPEG (低遅延フォールバック)** — canvas→toDataURLを`<img>`描画。ゲストの能力(`MediaSource.isTypeSupported`)をsubscribe時に申告しホストが視聴者ごとに切替。チャット(`relay:chat`)・リモート操作入力(`relay:input`)・操作許可バッジ(`relay:control_allowed`)もWS。**音声リレー**: ホストのシステム音声をMediaRecorder(webm/opus 48kbps)でチャンク送信→ゲストはwebm MSEで再生。視聴者subscribe時のみ送信=WebRTC全員対応なら零コスト。自動判定: `typeof RTCPeerConnection === 'undefined'`。**実機検証(2026-09-13)**: デスクトップ(Windows host) × Ubuntu 24.04 VM (guest) で guest passed:true — フレーム約12.7fps着信・チャット往復・操作バッジを確認(H264/MSE+音声モードの検証は別途)
...
### ⚠️ Known Issues
*   **Linux (Ubuntu/Debian系) はWebRTC不可 → WSリレーモードで対応**: ディストリ公式WebKitGTKがWebRTC無効ビルド (`typeof RTCPeerConnection === 'undefined'`)。GStreamerプラグイン追加でも回復しない=エンジン層の欠如。リレーモードで映像受信・チャット・リモート操作は可能(音声のみ未対応)。MSEでのH264リレーが次の品質向上候補
