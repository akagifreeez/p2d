...
**多人数実機テスト (pve VM3台×3視聴者+デスクトップホスト=9視聴者/10ノード)**: 9/9視聴者 passed — 全員がH264チャンク受信+署名検証 verified=13/invalid=0。木も成立 (5直結+中継1に4人割り当て、昇格→relay_ready→assignの一連が実機動作)。**判明した既知課題**: ①同時参加ストームでsubscribeがhostのpeer:joined処理に先行して破棄される → 鍵受信までの再送 (2/5/9/14s) で回収 ②WS再接続で新クライアントIDが振られ、コーディネータの木登録にゴーストが残り中継のfan-out上限が実効的に超過する (機能には影響せず、削除済みIDの期ja掃除で対処予定) ③host自身の内蔵ルームが空になると削除されるため、全直結視聴者が中継へ移った瞬間に新規joinがROOM_NOT_FOUNDになるケース → hostの常駐メンバーシップ保証が必要。→ **計画書: [配信木計画書.md](配信木計画書.md)**
