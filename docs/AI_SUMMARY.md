...
**多人数実機テスト (pve VM3台×3視聴者+デスクトップホスト=9視聴者/10ノード)**: 9/9視聴者 passed — 全員がH264チャンク受信+署名検証 verified=13/invalid=0。木も成立 (5直結+中継1に4人割り当て、昇格→relay_ready→assignの一連が実機動作)。**帯域A/B実測は未完了** — 動体ソースが必要だが、テスト中にデスクトップのディスプレイが電源Offになり、エンコードが停止したため。再実行手順 (全て `C:/tmp/` に完備): ①ディスプレイが起きた状態で `p2d-motion.ps1` (描画はウィンドウ内。ウィンドウ移動方式はtao 0.35.3をクラッシュさせる) ②host `--p2d-tree-fanout=4` で9視聴者参加 → デスクトップNIC送信バイトを30秒計測 (=直結本数×レート) ③ `--p2d-tree-fanout=99` で同測定 (全直結メッシュ) → A/B比較。視聴者側は各VMの `/proc/net/dev` rx。※`p2d.exe` のtaskkill後は `p2d_bin` 名の残プロセスにも注意。
