...
4. **さらに先: 配信木(オーバーレイマルチキャスト)** — 視聴者が子を担当してインフラゼロでスケール。離脱時の再接続(サブツリー付け替え)が勝負所。インフラゼロ哲学を貫る場合のみ → **計画書: [配信木計画書.md](配信木計画書.md)** (2026-09-13作成・未着手。発動条件「同一上り回線から5人以上への配信ユースケースが実発生したら」)
